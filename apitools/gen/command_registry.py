...
class CommandRegistry(object):
    ...
    def __PrintFlagDeclarations(self, printer):
        ...
        with printer.Indent():
            ...
            with printer.Indent('    '):
                ...
                printer(
                    "'Filename for a JSON service account key downloaded from '")
    ...
    def __PrintGetClient(self, printer):
        ...
        with printer.Indent():
            ...
            with printer.Indent():
                printer(
                    'client = client_lib.%s(', self.__client_info.client_class_name)
    ...
    def __PrintCommands(self, printer):
        ...
        for command_info in self.__command_list:
            ...
            with printer.Indent():
                printer(
                    '"""Command wrapping %s."""', command_info.client_method_path)
                printer()
                printer('usage = """%s%s%s"""',
                        command_info.name,
                        ' ' if arg_list else '',
                ...
                with printer.Indent():
                    printer(
                        'super(%s, self).__init__(name, fv)', command_info.class_name)

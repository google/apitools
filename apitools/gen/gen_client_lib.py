...
class DescriptorGenerator(object):
    ...
    def WriteSetupPy(self, out):
        ...
        apitools_version = _ApitoolsVersion()
        ...
        with printer.Indent(indent='    '):
            if apitools_version.startswith('0.4'):
                printer('"google-apitools>=0.4.8,<0.5",')
            else:
                printer('"google-apitools==%s",', apitools_version)
        ...
        with printer.Indent(indent='    '):
            ...
            if apitools_version.startswith('0.4'):
                ...
                printer('version="%s.%s",', apitools_version, self.__revision)
